...
class RescheduleResult(BaseModel):
    ...
    status: str = Field(default="success", description="状态: success/failed/skipped")
    ...
    old_queue_number: Optional[int] = Field(default=None, description="原队列号")
    old_equipment_id: Optional[int] = Field(default=None, description="原设备ID")
    new_date: Optional[date] = Field(default=None, description="新日期")
    new_time_slot: Optional[str] = Field(default=None, description="新时段")
    new_hospital_id: Optional[int] = Field(default=None, description="新院区ID")
...
class DrugDelayRequest(BaseModel):
    ...
    affected_date: Optional[date] = Field(default=None, description="受影响日期")
    target_date: Optional[date] = Field(default=None, description="目标改期日期")
    target_hospital_id: Optional[int] = Field(default=None, description="目标院区ID")
...
class BatchRescheduleResult(BaseModel):
    ...
    total: int = Field(..., description="总记录数(别名)")
    success: int = Field(..., description="成功数(别名)")
    failed: int = Field(..., description="失败数(别名)")
    skipped: int = Field(default=0, description="跳过数(别名)")
    reason: RescheduleReason = Field(..., description="改期原因")
    strategy: RescheduleStrategy = Field(..., description="重排策略")
    results: List[RescheduleResult] = Field(..., description="详细结果")
    success_details: List[Dict[str, Any]] = Field(default_factory=list, description="成功详情(含新日期、新时段、通知记录)")
    failed_details: List[Dict[str, Any]] = Field(default_factory=list, description="失败详情")
    skipped_details: List[Dict[str, Any]] = Field(default_factory=list, description="跳过详情")
